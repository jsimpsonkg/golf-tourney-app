...
export const getTeamsByTournament = async (tournamentId: string): Promise<Team[]> =>
	(await db.select().from(teams).where(eq(teams.tournament_id, tournamentId))).map(toTeam);

export const getPlayersByTournament = async (tournamentId: string): Promise<Player[]> =>
	(await db.select().from(players).where(eq(players.tournament_id, tournamentId))).map(toPlayer);

export const getSessionsByTournament = async (tournamentId: string): Promise<Session[]> =>
	(
		await db
			.select()
			.from(sessions)
...
export const getMatchesBySessions = async (sessionIds: string[]): Promise<Match[]> =>
	sessionIds.length
		? (await db.select().from(matches).where(inArray(matches.session_id, sessionIds))).map(toMatch)
		: [];
...
export const getParticipantsByMatch = (matchId: string): Promise<MatchParticipant[]> =>
	getParticipantsByMatches([matchId]);

/** Every recorded score for a match, hole order. */
export const getScoreEntriesByMatch = async (matchId: string): Promise<ScoreEntry[]> =>
	(
		await db
			.select()
			.from(score_entries)
...
export const getCourseHoles = async (tournamentId: string): Promise<CourseHole[]> =>
	(
		await db
			.select()
			.from(course_holes)
 ...
 */
export const upsertScore = async (input: ScoreUpsert): Promise<ScoreEntry | null> => {
